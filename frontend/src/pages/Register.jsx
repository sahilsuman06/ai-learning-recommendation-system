...
import { Mail, Lock, User, UserPlus, AlertCircle, Loader2, BookOpen } from 'lucide-react';
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Auto login for great user experience
      const loginRes = await API.post('/login-json', { email, password });
      const { access_token } = loginRes.data;
      localStorage.setItem('token', access_token);

      // 3. Fetch profile and save user data
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center px-4 relative overflow-hidden">
      {/* Glow Effects */}
      <div className="absolute top-1/4 right-1/4 translate-x-1/2 -translate-y-1/2 w-80 h-80 rounded-full bg-emerald-500/10 blur-[100px] pointer-events-none"></div>
      <div className="absolute bottom-1/4 left-1/4 -translate-x-1/2 translate-y-1/2 w-80 h-80 rounded-full bg-teal-500/10 blur-[100px] pointer-events-none"></div>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-400 mt-2 text-sm">Sign up to unlock personalized AI learning pathways</p>
        </div>
